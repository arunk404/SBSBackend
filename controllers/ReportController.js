    ...
    form.parse(req, async function (error, fields, file) 
    {
      try {
        ...
        if(fields.report_type_id[0] == 1){      // Checklist Report 
            ...
            // return res.status(400).json({ code: 400, status: 'Validation Error1', message: "Records is required" });
    ...
    form.parse(req, async function (error, fields, file) 
    {
      try {
        ...
        if(fields.report_type_id[0] == 1){      // Checklist Report 
            ...
            const data1 = await db.sequelize.query('CALL getReportById(:id)', {
                replacements: {
        ...
        res.status(201).json({code: 201, status:'Success', message:'Report Update Successfully!', successid: newObject[0].id});
        res.end();
      } catch (error) 
      {
        console.log(error);
